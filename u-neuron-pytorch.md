...
- ROUND development team — blocked on empirical validation
...
| API surface | `src/round/__init__.py` | Public exports |
...
- **mypy** — type checking via `mypy src/round/`
...
4. Add `UTensor` to `src/round/__init__.py` exports
...
- ✅⚙️ Immediate/Mechanical: `from round import UTensor` succeeds; `UTensor(x, eps)` returns object with correct shapes
...
4. Create `tests/test_norm.py` with tests for:
   ...
   - Shape mismatch raises ValueError
5. Add `u_norm`, `u_distance` to `src/round/__init__.py`
...
4. Add `ULinear` to `src/round/__init__.py`
...
1. Create `src/round/emission.py`
...
6. Add `u_emit`, `UEmission` to `src/round/__init__.py`
...
1. Create `src/round/regularization.py`
...
4. Add `LandauerRegularizer` to `src/round/__init__.py`
...
4. Add `UModel` to `src/round/__init__.py`
...
|----|-----------|------|-----------|
| SC01 | `pip install -e .` succeeds from `projects/round/` | ⚙️ Mechanical | Immediate |
| SC02 | `from round import UTensor, ULinear, UEmission, UModel, u_norm, u_distance, LandauerRegularizer` succeeds | ⚙️ Mechanical | Immediate |
| SC03 | `pytest tests/ -v` shows ≥50 test cases, all passing | ⚙️ Mechanical | Immediate |
| SC04 | `mypy src/round/` passes with 0 errors | ⚙️ Mechanical | Immediate |
...
4. **Complex multiplication is a known technique.** Complex-valued neural networks exist (Trabelsi et al., 2018). The novelty of ROUND is NOT in the complex multiplication itself, but in: (a) the information-theoretic motivation (Landauer regularization), (b) the asymmetric scale initialization (W_b at infinitesimal scale), and (c) the interpretation of ε as a curvature selector within a foliated non-Archimedean fiber bundle. These distinguish ROUND from generic complex NNs.
